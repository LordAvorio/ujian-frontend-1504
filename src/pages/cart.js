...
class Cart extends Component {
    ...
    //#region Handle Edit
        ...
        handlePlusKuantiti = (index) => {
            ...
            if(this.state.kuantitiBaru >= tempProduk[tempIndex].stock-1){
                return swal("Ouch!", "Data tidak boleh lebih dari stok !", "error")
            }else{
                this.setState({
        ...
        handleMinusKuantiti = () => {
            if(this.state.kuantitiBaru <= 0){
                return swal("Ouch!", "Data tidak boleh kurang dari 0!", "error")
            }else{
                this.setState({
                    kuantitiBaru: this.state.kuantitiBaru - 1
                })
    ...
    deleteCart = (index) => {
        ...
        .then((res) => {
            ...
            Axios.get(`http://localhost:2000/users/${localStorage.id}`)
            .then((res) => {
                swal("Yuhuu!", "Data berhasil dihapus!", "success")
                this.props.login(res.data)
            })
            .catch((err) => {
                console.log(err)
    ...
    //#region Checkout Pembayaran

    checkOut = () => {
        if(this.props.cart.length === 0) return swal("Ouch!", "Data Cart Masih Kosong !", "error")
        ...
        return (
            <div>
                    ...
                    <Row style={{padding:"8% 5%"}}>
                        <Col md={12}>
                            <Row>
                                ...
                                <Col md={6} style={{textAlign: "right", color: "#1fab89"}}>
                                    <Button variant="primary" style={{width:"40%"}} onClick={() => this.checkOut()}>
                                        CHECKOUT
                                    </Button>
                                </Col>
                            </Row>
                ...
                <Modal show={this.state.confirmationBayar} onHide={() => this.setState({ confirmationBayar: false })}>
                    ...
                    <Modal.Body>
                        <p>Apakah anda yakin mau membeli barang ?</p>
                    </Modal.Body>
                    <Modal.Footer>
                        <Button variant="secondary" onClick={() => this.setState({ confirmationBayar: false })}>
                            Close
                        </Button>
...
// return this.state.dataUserCart.map((item, index) => {
//     return (
//         <tr key={item.id}>
//             <td style={{textAlign: "center"}}>{index + 1}</td>
//             <td>{item.name}</td>
...
//                     </Col>
//                     <Col md={6} style={{textAlign: "center"}}>
//                         <Button variant="danger" onClick={() => this.deleteCart(item.id)}>Delete</Button>
//                     </Col>
//                 </Row>
//             </td>
//         </tr>
...
// componentDidMount = () => {
//     Axios.get(`http://localhost:2000/users?id=${localStorage.id}`)
//     .then((res) => {
//         this.setState({
//             dataUserCart: res.data[0].cart                 
//         })
//     })
//     .catch((err) => {
//         console.log(err)
//     })
 ...
 // var tempStok = 0
        ...
        // for(let i=0;i<tempProduk.length;i++){
        //     if(tempCart.id === tempProduk[i].id){
        //         for(let j=0; j<tempProduk[i].stock.length; j++){
        //             if(tempCart.size === tempProduk[i].stock[j].code){
        //                 tempStok += tempProduk[i].stock[j].total
...
//#region Ubah Stok
        ...
        // for(let i=0;i<tempProduk.length;i++){
        //     if(tempProduk[i].id === tempCart.id){
        //         for(let j=0;j<tempProduk[i].stock.length;j++){
        //             if(tempProduk[i].stock[j].size === tempCart.size){
        //                 tempProduk[i].stock[j].total = tempProduk[i].stock[j].total - tempCart.qty
        //                 this.setState({
        //                     dataProducts: tempProduk
        //                 })
        //             }
        //         }
        //     }
        ...
        // Axios.patch(`http://localhost:2000/products/${tempCart.id}`, {stock: tempProduk.stock})
        // .then((res) => {
        //     console.log(res.data)
        // })
        // .catch((err) => {
