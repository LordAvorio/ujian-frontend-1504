...
class Login extends Component {
    ...
    handleLogin = () => {
        ...
        if(!email || !password) return swal("Ouch!", "Input Field First", "error");

        Axios.get(`http://localhost:2000/users?email=${email}&password=${password}`)
        .then((res) => {
            if(res.data.length === 0) return swal("Ouch!", "Email / Password is wrong", "error");
            
            swal("Yuhuu!", "Anda Berhasil Login", "success");
