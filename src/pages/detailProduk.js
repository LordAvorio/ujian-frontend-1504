...
class DetailProduk extends Component {
    ...
    handleAddToCart = () => {
        if(!this.props.email){
            return(
                swal("Ouch!", "You Must Login First", "error"),
                this.setState({toLogin: true}) 
            )
        }else if(this.state.total === 0){
            return swal("Ouch!", "Please choose quantity!", "error") 
        }else{
            let cartData = {
    ...
    render() {
        ...
        return (
            <div>
                <Container>
                    <Row>
                        <Col md={6}>
                            <Row>
                                <Col md={12}>
                                    <Image src={this.state.detailProduk.img} rounded style={{width: "100%"}} />
                                </Col>
                            </Row>
                        </Col>
                        <Col md={6}>
                            ...
                            <Row>
                                <Col md={6}>
                                    <Row>
                                        ...
                                        <Col md={6}>
                                            <p style={{fontSize: "25px", textAllign: "center"}}> {this.state.total}</p>
                                        </Col>
                                        <Col md={3}>
                            ...
                            <Row>
                                <Col md={12}>
                                    <Button style={{width: "100%"}} onClick={() => this.handleAddToCart()} >Add To Cart</Button>
                                </Col>
                            </Row>
                        </Col>
                    </Row>
                ...
                <Toast show={this.state.cartAdded} onClose={() => this.setState({cartAdded: false})}>
                    <Toast.Header>
                        <strong className="mr-auto">Bootstrap</strong>
                        <small>Cart is Added</small>
                    </Toast.Header>
                    <Toast.Body>
                        <p>Cart berhasil dimasukkan</p>
                        <Button onClick={() => this.setState({cartAdded: false})}>Tutup</Button>
                    </Toast.Body>
                </Toast>
            </div>
        )
